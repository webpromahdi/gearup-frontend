    ...
    <aside className="hidden fixed inset-y-0 left-0 z-40 w-[260px] flex-col bg-[#1b2748] px-4 py-7 text-white lg:flex">
      ...
      <div className="flex items-center gap-3 border-b border-white/15 pb-6">
        ...
        <div>
          ...
          <span className="mt-1 inline-flex rounded-full bg-[#e31824] px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-[0.1em]">
            Customer
          </span>
        </div>
      </div>
        ...
        {nav.map(([Icon, label, href]) => {
          ...
          return (
            <Link
              key={label as string}
              href={href as string}
              className={`flex items-center gap-3 rounded-lg border-l-2 px-3 py-3 text-sm font-semibold transition ${
                isActive
      ...
      <button
        ...
        className="mt-auto flex h-11 items-center justify-center gap-2 rounded-lg border border-[#e31824] text-sm font-bold text-red-200 transition hover:bg-[#e31824] hover:text-white"
      >
...
function DashboardMobileBar() {
  ...
  return (
    <header className="flex h-16 items-center justify-between bg-[#1b2748] px-5 text-white lg:hidden">
      ...
      <button onClick={handleLogout} className="text-sm font-bold text-red-200">
        Exit dashboard
      </button>
    </header>
  );
